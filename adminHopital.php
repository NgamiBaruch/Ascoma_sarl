...
<?php
$rep = $con->prepare("SELECT * FROM hopistal");
try {
    $rep->execute();
...
<div class="container my-5">
    <div class="row">
        <div class="col-md-12">
            <a href="inscritHopital.php" class=" btn btn-primary mt-lg-5"> new Hospital</a><br><br>
            <div class=" table-responsive">
                <table id="mytable" class="table table-success table-responsive table-striped">
                    ...
                    <tbody>
                    <tr>
                        <td><?=htmlentities($row->IdHopistal)?></td>
                        <td><?=htmlentities($row->Nom)?></td>
                        <td><?=htmlentities($row->DecretCreation)?></td>
                        <td><?=htmlentities($row->Email)?></td>
                        <td><?=htmlentities($row->Telephone)?></td>
                        <td><?php echo htmlentities($row->Code) ?></td>
                        <td><?php echo htmlentities($row->Password) ?></td>
                       
                        <td> <a href="modifihopital.php?IdHopistal=<?=htmlentities($row->IdHopistal)?>" class="btn btn-outline-primary btn-sm" > Edit</a>
                            <a href="adminHopital.php?IdHopistal=<?=htmlentities($row->IdHopistal)?>" class="btn btn-danger"  onclick="return confirm ('veux-tu vraiment supprimer ?')"> Delete</a>
